...
import { supabase, getCurrentUser, signOut } from './shared/services/supabase';
...
// Mobile imports (web versions)
import MobileHome from './mobile/screens/HomeScreen';
import MobileHistory from './mobile/screens/HistoryScreen';
import MobileProfile from './mobile/screens/ProfileScreen';
  ...
  return (
    <Router>
      <div className="min-h-screen bg-background font-body-md text-on-surface">
        <Routes>
          {/* Public Routes */}
          <Route path="/" element={!user ? <Navigate to="/login/admin" /> : <Navigate to="/admin" />} />
          <Route path="/login/admin" element={!user ? <Login onLogin={checkUser} mode="admin" /> : <Navigate to="/admin" />} />
          <Route path="/login/mobile" element={!user ? <Login onLogin={checkUser} mode="mobile" /> : <Navigate to="/mobile" />} />
          ...
          <Route
            ...
            element={
              user ? (
                <AdminLayout user={user} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} handleLogout={handleLogout}>
                  <Routes>
...
function AdminLayout({ children, user, sidebarOpen, setSidebarOpen, handleLogout }: any) {
  return (
    <>
      {/* Sidebar Desktop */}
      <aside className="hidden md:flex flex-col h-full w-64 fixed left-0 top-0 bg-surface-container border-r border-outline-variant shadow-md z-50 p-md gap-sm">
        ...
        <nav className="flex-1 flex flex-col gap-xs">
          ...
          <SidebarLink to="/admin/attendance">
      ...
      <aside className={`md:hidden fixed left-0 top-0 h-full w-64 bg-surface-container border-r border-outline-variant shadow-lg z-50 flex flex-col p-md gap-sm transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 flex flex-col gap-xs">
          <SidebarLink to="/admin" onClick={() => setSidebarOpen(false)}>
            <span className="material-symbols-outlined">dashboard</span>
            Painel
          </SidebarLink>
          <SidebarLink to="/admin/locations" onClick={() => setSidebarOpen(false)}>
            <span className="material-symbols-outlined">location_on</span>
            Locais
          </SidebarLink>
          <SidebarLink to="/admin/users" onClick={() => setSidebarOpen(false)}>
            <span className="material-symbols-outlined">group</span>
            Colaboradores
          </SidebarLink>
          <SidebarLink to="/admin/attendance" onClick={() => setSidebarOpen(false)}>
            <span className="material-symbols-outlined">history</span>
            Histórico
          </SidebarLink>
        </nav>
