...
export default function Login({ onLogin, mode }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onLogin();
      navigate(mode === 'admin' ? '/admin' : '/mobile');
    } catch (err: any) {
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 className="font-headline-md text-headline-md text-primary" style={{ marginBottom: 4 }}>
            Precision Attendance
          </h1>
          <p className="font-body-sm text-body-sm text-on-surface-variant">
            {isMobile ? 'Registro de Ponto' : 'Painel Administrativo'}
          </p>
        </div>
